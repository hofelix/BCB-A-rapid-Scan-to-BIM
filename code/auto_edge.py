...
K1 = 50
r = np.pi / 12
load_path = os.path.abspath('D:\Program\RANSAC_EDGE\ply_index')
save_path = os.path.abspath('D:\Program\RANSAC_EDGE\edge_index')

def edge(file_name):
    import_path = os.path.join(load_path,file_name)
    export_path = os.path.join(save_path,file_name)
    pcd = o3d.io.read_point_cloud(import_path)
    ...
    boundary_threshold = c2_mean + 2*c2_std
    for i in range(pcd_size):
    ...
    R = inlier_pcd.get_rotation_matrix_from_xyz((-1*r, -1*r, -1*r))
    inlier_pcd.rotate(R, center=(0, 0, 0))
    inlier_pcd.paint_uniform_color([1, 0, 0])
    o3d.io.write_point_cloud(export_path, inlier_pcd)
