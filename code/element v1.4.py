...
def outlier_edge_check(xyz,full_point,parallel_list):
    num = np.shape(xyz)[0]
    ...
    for i in a:
        fits = np.asarray(i)
        fit = xyz[fits,:]
        c = circle(fit[0,:],fit[1,:],fit[2,:])
        other = np.setdiff1d(lists, fits)
        for j in other:
            s_dis = dis_p_to_s(c.s, xyz[j,:])
            r_dis = abs(dis_p_to_p(xyz[j,:], c.center) - c.radius)
            if s_dis <= 0.05 and r_dis <= 0.05:
                fits = np.append(fits,j)
        inlier_list.append(fits)
    inlier_num = np.zeros((len(a)))
    for i in range(len(a)):
        inlier_num[i] = np.shape(inlier_list[i])[0]
    max = np.max(inlier_num)
    maxn = np.where(inlier_num == max)[0][0]
    if max > 3:
        inlier = np.array(inlier_list[maxn],dtype=int)
        parallel_list = parallel_list[inlier]
        inlier = np.append(inlier,inlier + int(max))
        full_point = full_point[inlier,:]
    return full_point,parallel_list
...
def fake_tri_check(point,all_point,parallel_list):
    ...
    if minist <= 0.09:
        print('one point is close to another ',minist)
...
def polygon_reconstruction(parallel_list, vertex_list, vector_list, edge_num):
    ...
    lists = np.linspace(0, num, num, endpoint=False,dtype = int)
    if num > 4:
        xyz,parallel_list = outlier_edge_check(xyz[lists,:],xyz,parallel_list)
    num = int(0.5*(np.shape(xyz)[0]))
    lists = np.linspace(0, num, num, endpoint=False,dtype = int)
    if num > 4:
        xyz,parallel_list = outlier_edge_check(xyz[lists+num,:],xyz,parallel_list)
    xyz = section_check(xyz)
    p = vertex_to_mesh(xyz)
    return p.center, p.hull,parallel_list
...
def boundary_repair(vertex_list, vector_list,num):
    ...
    dis_list = np.zeros((num))
    for i in range(num):
        dis_list[i] = dis_ve_to_c(vertex_list[i, :], vector_list[i, :], center)*-1
    l = np.max(dis_list)
    for i in range(num):
